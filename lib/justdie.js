 ...
 */
let sum_pool = (...dies) => dies.reduceRight((prev_roll, die) => die() + prev_roll, 0);
 ...
 */
let rand_range = (min, max) => Math.floor(Math.random() * ((max + 1) - min)) + min;
 ...
 */
let roll = (...dice) => dice.length === 1 ? dice() : sum_pool(...dice);
 ...
 */
let roll_verbose = (...dice) => dice.length === 1 ? verbose(dice) : (...dice) => dice.map(die => verbose(die));
